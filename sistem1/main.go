...
func (state *AggregatorActor) Receive(ctx actor.Context) {
	...
	case *messages.LocalWeights:
		...
		if state.unprocessedRemoteWeights {
			...
			state.localWeights.Weights = averageWeights(state.remoteWeights.Weights, converter.ConvertFromProtoWeights(msg.Weights), 1)
			state.unprocessedRemoteWeights = false
		} else {
			var localWeights model.LocalWeights
			localWeights.Weights = converter.ConvertFromProtoWeights(msg.Weights)
			state.localWeights = localWeights
			fmt.Println("No new remote weights")
		}
		ctx.Send(state.trainingPID, state.localWeights)
	case *messages.RemoteWeights: //ako ima stranih usrednji (setuje fleg samo dok ne dobije lokalne)
		fmt.Println("Aggregator received remote weights")
		state.unprocessedRemoteWeights = true
		state.remoteWeights.Weights = converter.ConvertFromProtoWeights(msg.Weights)
...
func (state *CommunicationActor) Receive(ctx actor.Context) {
	...
	case *messages.LocalWeights:
		if state.aggregatorPID.Address == "127.0.0.1:8081" {
			fmt.Println("sistem1: Communication actor received weights from training actor")
		}
		if state.aggregatorPID.Address == "127.0.0.1:8082" {
			fmt.Println("sistem2: Communication actor received weights from training actor")
		}
		...
		if state.aggregatorPID.Address == "127.0.0.1:8081" {
			fmt.Println("sistem1: Send weights to other systems")
		}
		if state.aggregatorPID.Address == "127.0.0.1:8082" {
			fmt.Println("sistem2: Send weights to other systems")
		}
	...
	case *messages.RemoteWeights:
		if state.aggregatorPID.Address == "127.0.0.1:8081" {
			fmt.Println("sistem1: RECIEVED WEIGHTS FROM ANOTHER NODE")
		}
		if state.aggregatorPID.Address == "127.0.0.1:8082" {
			fmt.Println("sistem2: RECIEVED WEIGHTS FROM ANOTHER NODE")
		}
	...
	case *messages.AggregatorPIDMsg:
		state.aggregatorPID = converter.ConvertProtoToActorPID(msg.AggregatorPID)
	case *messages.OtherCommunicationPIDMsg:
		state.otherCommunicationPID = converter.ConvertProtoToActorPID(msg.OtherCommPID)
...
func main() {
	...
	//pravljenje komunikacionog aktora
	...
	aggregatorPIDMsg := &messages.AggregatorPIDMsg{
		AggregatorPID: converter.ConvertActorToProtoPID(aggregatorPID),
	}

	otherCommunicationPIDMsg := &messages.OtherCommunicationPIDMsg{
		OtherCommPID: converter.ConvertActorToProtoPID(otherCommPID),
	}
	...
	trainingProps := actor.PropsFromProducer(func() actor.Actor {
		return &TrainerActor{commActorPID: commPID}
	})
	...
	rootContext.Send(dataloaderPropsPID, LoadDataRequest{})

	rootContext.Send(aggregatorPID, model.TrainingPIDMsg{TrainingPID: trainingPID})

	for !endProgram || c.MemberList.Length() >= 1 {
